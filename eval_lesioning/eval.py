...
def make_random_selections(selections_dir_random, selections_dir, num_units, greedy_p):
    ...
    selections_random_filename = os.path.join(selections_dir_random, 'selections_record.npz')
    selections_random_filename_data = np.load(selections_random_filename, allow_pickle=True)

    lst = selections_random_filename_data.files

    for item in lst:
        print(item)
        print(selections_random_filename_data[item])
    
    if not os.path.exists(selections_dir):
            os.makedirs(selections_dir)


    c = list(zip(selections_random_filename_data[lst[0]], selections_random_filename_data[lst[1]], selections_random_filename_data[lst[2]]))
    random.shuffle(c)
    selected_units, selected_losses, selected_accuracies = zip(*c)
    ...
    np.savez(selections_filename, 
             ...
             selected_subperformances=selections_random_filename_data[lst[3]],
             selections_complete=selections_random_filename_data[lst[4]]
...
def get_evaluation_objects(config_file,
    ...
    with open(config_file) as f:
        config = yaml.load(f)
    max_valid_samples = copy.deepcopy(config["max_samples"]["valid"])
    if nonevaluation_task is not None:
        max_valid_samples[nonevaluation_task] = 0
    validator = transfer.Validator(name=evaluation_task +'_validator', 
                                 model=model, 
                                 batch_size=batch_size,
                                 data_dir=list(config["data"].values()), 
                                 data_subdir='test',
    ...
    if randomize_classes:
        assert(config["project"]["num_tasks"] > 1), "Only single task, cannot randomize classes!"
...
def run_evaluator():
    ...
    model = lesion.get_model(config=config, 
                             ngpus=FLAGS.ngpus, 
                             dataParallel=True, 
                             pretrained=True)
    ...
    if FLAGS.random_lesion:
        ...
        evaluation_sort_task = sort_task
        evaluation_nonsort_task = None
        if config["project"]["num_tasks"] > 1:
            evaluation_nonsort_task = nonsort_task

    selections_dir_random=os.path.join(records_dir,
                                'selections_records','SORTEDBY_'+sort_task,
    ...
    if config["project"]["num_tasks"] == 1:
        drop_percents_dir_nonsort_task = os.path.join(records_dir,
                                                      'drop_percents_records', 
                                                      'VERSION_' + FLAGS.eval_version ,
                                                      'EVALUATION_TASK_' + str(evaluation_nonsort_task),
                                                      'SORTEDBY_' + task_to_sort_by,
                                                      'PARAM_GROUP_INDEX_'+ str(FLAGS.param_group_index))
    else:
        drop_percents_dir_nonsort_task = None
    ...
    if FLAGS.random_lesion:
        make_random_selections(selections_dir_random = selections_dir_random, selections_dir=selections_dir, num_units=num_units, greedy_p=FLAGS.greedy_p)
    ...
    if nonsort_task is None:
        return None # Exit since there is no nonsort task
