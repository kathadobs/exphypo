...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_epochs',    default=99,    type=int,  help='num of epochs to train for')
    parser.add_argument('--valid_freq',    default=0.5,   type=float, help='how often to validate as percentage of epoch')
    parser.add_argument('--save_freq',     default=10,     type=int,     help='how often to save, dafault ever 5 epochs ')
    ...
    parser.add_argument('--use_scheduler', default=True, type=bool, help='if True will use scheduler')
    ...
    if FLAGS.basemodel is None:
        base_network_name = 'vgg/inanimate_vgg_large'
    else:
        base_network_name = FLAGS.basemodel
    config_base = Config(config_file='./configs/' + base_network_name + '.yaml')

    # IMPORT MODEL Using Custom
    n_in = 4096
    n_out = 1109

    _, ckpt_data = config_base.get_model(pretrained=True, ngpus=FLAGS.ngpus, dataParallel=True, epoch=-1)
    model, _ = config_base.get_model(pretrained=False, ngpus=FLAGS.ngpus, dataParallel=False)

    # model.module.classifier = nn.Sequential(*list(model.module.classifier.children())[:4])

    # model['state_dict']['module.classifier.6.weight'] = torch.Tensor(n_out, n_in).to("cuda")
    # model['state_dict']['module.classifier.6.bias'] = torch.Tensor(n_out).to("cuda")
    ...
    # MODIFY CLASSIFIER
    model.classifier[6] = torch.nn.Linear(in_features=n_in, out_features=n_out, bias=True)


    print('setting requires_grad')
    ...
    if config.optimizer == 'adam':
        trainer.optimizer = torch.optim.Adam(params=model.parameters(), 
                                             ...
                                             weight_decay=config.weight_decay, 
                                             amsgrad=False)
    # changed from params=model.module.classifier.parameters() to
    # params=model.parameters()
    elif config.optimizer == 'sgd':
        trainer.optimizer = torch.optim.SGD(params=model.parameters(),
                                                 lr=config.learning_rate,
                                                 momentum=config.momentum,
                                                 weight_decay=config.weight_decay)
