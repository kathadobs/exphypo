...
def get_bootstrap_acc_diff(y_true_face, y_pred_face, y_true_obj, y_pred_obj):

    rand_classes_face = np.unique(y_true_face)
    n_classes_face = rand_classes_face.shape[0]

    rand_classes_obj = np.unique(y_true_obj)
    n_classes_obj = rand_classes_obj.shape[0]
    ...
    for iBoot in range(n_boot):
        ...
        # bootstrap classes
        bootstrap_classes_face = np.random.choice(rand_classes_face, size=n_classes_face, replace=True)
        bootstrap_classes_obj = np.random.choice(rand_classes_obj, size=n_classes_obj, replace=True)

        for i,iClass_face in enumerate (bootstrap_classes_face):
            class_images_face = np.where(y_true_face==iClass_face)[0]

            bootstrap_images_face = np.random.choice(class_images_face, size=class_images_face.shape[0], replace=True)

            if i==1:
                y_true_boot_face = np.ones(shape=(class_images_face.shape[0],))*iClass_face
                y_pred_boot_face = y_pred_face[bootstrap_images_face]
            else:
                y_true_boot_face = np.hstack((y_true_boot_face, np.ones(shape=(class_images_face.shape[0],))*iClass_face))
                y_pred_boot_face = np.hstack((y_pred_boot_face, y_pred_face[bootstrap_images_face]))

        acc_face = np.where(y_true_boot_face == y_pred_boot_face)[0].shape[0]/y_true_boot_face.shape[0]

        for i,iClass_obj in enumerate (bootstrap_classes_obj):
            class_images_obj = np.where(y_true_obj==iClass_obj)[0]

            bootstrap_images_obj = np.random.choice(class_images_obj, size=class_images_obj.shape[0], replace=True)

            if i==1:
                y_true_boot_obj = np.ones(shape=(class_images_obj.shape[0],))*iClass_obj
                y_pred_boot_obj = y_pred_obj[bootstrap_images_obj]
            else:
                y_true_boot_obj = np.hstack((y_true_boot_obj, np.ones(shape=(class_images_obj.shape[0],))*iClass_obj))
                y_pred_boot_obj = np.hstack((y_pred_boot_obj, y_pred_obj[bootstrap_images_obj]))

        acc_obj = np.where(y_true_boot_obj == y_pred_boot_obj)[0].shape[0]/y_true_boot_obj.shape[0]
        
        acc_boot[iBoot] = acc_face - acc_obj
